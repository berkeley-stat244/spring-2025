...
### Uses of CuArray objects

Only some of the operations you might try to do are implemented for the CuArray type.

```{julia}
#| eval: false
y_gpu = CuArray([1.1,3.3,4.4]);
y_gpu + y_gpu
```
```
...
```
```
ERROR: MethodError: no method matching sin(::CuArray{Float64, 1, CUDA.DeviceMemory})
```
...
The 2.0 ms is reasonably comparable to the 3.7 ms when we just had Julia run the vectorized computation on the GPU. That used 64-bit floats. When I reran the code above using 64-bit floats, the time was 5.2 ms.
...
Depending on how much time we have in future classes and the focus of student presentations and projects, we may come back to this topic to explore GPU-based operations from Julia, in particular GPU-based computations on CuArrays and writing more interesting kernels.