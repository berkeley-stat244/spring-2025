...
# 7.731 s (2 allocations: 1.86 GiB)
x_gpu = CuArray(x);
CUDA.@time CUDA.@sync vec_calc(x_gpu);
#  3.744 ms (105 allocations: 2.25 KiB)
```

Wow, that's a 2000-fold increase. It seems that a chunk of that may be because the CPU calculation allocates a new array to store the result, whereas the GPU calculation doesn't (but it would if we transferred the result back to the host).

Instead, let's compare to the time for a for loop implementation on the CPU.

```{julia}
#| eval: false

function loop_calc(x)
  n = length(x);
  for i in 1:n
...
@btime loop_calc(x);
# 9.463 s (0 allocations: 0 bytes)
```

Hmmm, it seems that the allocation was not the cause of the slowness! 
...
Threads:
 ...
 - threads in a warp operate in lock step (so an if statement can cause a threads to pause to wait for other threads).
...
Grids
 ...
 - A100 GPU has up to 2147483647 blocks per grid (I think)
...
Next we'll see that we can actually write a kernel - a function that can be run on the GPU - in Julia and then call that kernel. Kernels are functions that encode the core computational operations that are executed in parallel.
...
I have not investigated to see what Julia does behind the scenes with the kernel code for it to be able to run on the GPU. I don't know if there is a step in which it is translated to CUDA code, but I would guess there is.
...
We need to use at least as many threads as computations, and in addition to only being able to use 1024 threads in the x-dimension, we can have at most 1024 threads in a block on the GPU we're using. So we'll need multiple blocks.
...
CUDA.@sync @cuda threads=nthreads blocks=nblocks my_kernel(x_gpu);
...
The 2.0 ms is comparable to the 3.7 ms when we just had Julia run the vectorized computation on the GPU. That used 64-bit floats (check that), which may explain the slower speed compared to what is seen here.