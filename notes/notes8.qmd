...
Array(result)[1] -n*log(2*pi)/2  # Do t
...
I haven't looked into this fully, but when I asked a ChatBot whether I could use shared memory to more efficiently do the reduction within each thread block before doing the final reduction across blocks, this is the key part of the code it suggested. It uses a tree-like operation to compute the partial sum within each thread block before using the atomic operation to compute the sum of the partial sums:

```{julia}
#| eval: false
shmem = @cuDynamicSharedMem(Float32, blockDim().x)
if idx <= length(input)
    shmem[tidx] = -0.5*x[idx]^2;
else
    shmem[tidx] = 0.0;
end

# Tree-like partial sum within the block,
# summing pairs until the sum within the block
# is contained in `shmem[1]`.
s = blockDim().x ÷ 2;
while s > 0
    if thread_idx <= s
