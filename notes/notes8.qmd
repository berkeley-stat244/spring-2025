...
The main package we'll use is the [CUDA package](https://cuda.juliagpu.org/stable/). It provides:
 ...
 - wrappers for NVIDIA's CUDA library (distinct from the Julia CUDA package) functions that allow one to use Julia functions/operators with CuArrays, and
...
Digging around a bit in the source code for the `LinearAlgebra` and `CUDA` packages, it looks like the result of the dispatch is to call `LinearAlgebra.generic_matmatmul!`, with the method that works for CuArrays in the `lib/cublas/linalg.jl` file in the CUDA package. That presumably invokes the cuBLAS versions of the BLAS functions found in `lib/cublas/libcublas.jl` and `lib/cublas/wrappers.jl` where we see calls out to native cuBLAS functions. 
...
Working with objects on the GPU requires interfacing with functions provided by the CUDA Toolkit and its related packages such as `cuBLAS`, `cuRAND`, `cuFFT`, etc. These are provided for Julia with the CUDA package (see `CUDA/<version_string>/lib`) so all that you need installed on the machine with the GPU is the NVIDIA driver software and the Julia CUDA package. You don't actually need NVIDIA's CUDA Toolkit.

Only some operations are possible on the GPU, since Julia is calling out to CUDA functions to run code on the GPU.
...
```
```
ERROR: MethodError: no method matching sin(::CuArray{Float32, 1, CUDA.DeviceMemory})
```

And we can't work with scalar elements of the array because that tries to do a non-vectorized calculation on a GPU-based array.
...
# GPU calculation:
# Try to flush out any compilation/transformation time.
...
The choice of the grid/block arrangement can affect efficiency. I'm not an expert at this level of detail but we'll see some about this in the worked example below. Note that using more than 1-dimensional grids and blocks is purely for the conceptual convenience of the programmer and doesn't correspond to anything on the hardware. So for the most part we'll use a one-dimensional grid of blocks and a one-dimensional blocks of threads.
...
### Glossary
 ...
 - *kernel*: a C function that runs on the GPU
...
### GPU execution
 ...
 - GPUS are good at executing the same computation across many data elements at once.
 ...
 - Computations on a GPU generally use single precision (4 byte floats), which will often run much faster than with doubles (8 byte floats).
...
Here's an example with one of the SCF GPU servers, showing information about the processors available on the A100 GPU and the threads/blocks/grids that are possible (more on this later).
...
Here we'll run the computation we ran earlier when we did not write our own kernel and just relied on Julia to offload to the GPU behind the scenese.
...
Depending on how much time we have in future classes and the focus of student presentations and projects, we may come back to this topic to explore GPU-based operations from Julia, in particular writing more interesting kernels.