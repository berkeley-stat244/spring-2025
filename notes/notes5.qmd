...
That produces [this output](statprof/index.html), which can in some ways be hard to interpret, but the color-coded division between `inv`, `*` and `*` gives us an idea of where time is being spent. 
...
That indicates that it's better to pre-allocate memory in Julia, but the time does not seem to grow as order of $n^2$ as it does in R.
...
That's perhaps a bit surprising given one might think that because the multiplication is done first, the `σ .* randn.()` might produce a scalar.
