...
Some highlights from the [Julia style guide](https://docs.julialang.org/en/v1/manual/style-guide/) include:
 ...
 - Use capitalized/UpperCamelCase type and module names.
 - Use lower-case squashed names (with underscores/snake_case as needed) for function names (e.g., `isequal` or possibly `is_equal`).
 - Use `isa` and `<:` rather than `==`.
...
A package is the way that Julia code is distributed as a bundle of functionality. It contains source code files, tests, documentation, and metadata (e.g., about dependence on other packages).
...
If we want to access variables/functions from a file, we can use `include`, which will evaluate the file contents in the global scope.
...
There's a lot of detail in this section. It's not critical to absorb it at this point, but it's here for when you need it.
...
Note that this includes the standard library, the active project, and (in some cases) a system project.
...
Julia will often precompiled packages. This is separate from Just-in-time (JIT) compilation of code. I have not had a chance to understand what the precompilation is doing. 
...
You can use the package manager either in the `Pkg` REPL by pressing `]`, or by calling the `Pkg` API via `Pkg.`, after `using Pkg`.
...
We can activate (and create if needed) a project:
...
You can see the packages in a project:
...
Create a new project. Add some of the packages that we've used so far to the project (e.g, `Random`, `Distributions`). Figure out where the packages are installed on your filesystem.
:::
...
The Julia documentation provides a [nice overview of creating a package](https://julialang.org/contribute/developing_package/).
...
Note that because a Git repository was created, we need to commit any changes to `src` to have them reflected in the package
as we use it in Julia.
...
Here we use a recursion, but the same thing happens when the nested function calls are not recursive.
...
Users can then control the level of logging information that they see. In this example we set the logging level for warnings and more serious messages, so the Info message is not shown.
